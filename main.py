...
@app.route("/")
def home():
    return '<a href="/panel.html" style="font-family:sans-serif;font-size:20px;">тЮбя╕П Open Messenger Bot Panel</a>'
...
def upload():
    ...
    for key in ["cookies", "targets", "messages", "prefix"]:
        ...
        f.save(path)
        files[key] = path
    return "тЬЕ Files uploaded successfully", 200
...
def start_bot():
    ...
    with BOT_LOCK:
        if BOT_STATUS["running"]:
            return "тЪая╕П Bot already running", 400
        STOP_EVENT.clear()
        BOT_THREAD = threading.Thread(target=lambda: asyncio.run(bot_main()), daemon=True)
        BOT_THREAD.start()
    return "тЬЕ Bot started", 200


@app.route("/stop", methods=["POST"])
def stop_bot():
    STOP_EVENT.set()
    return "ЁЯЫС Stop signal sent", 200
...
try:
                    ...
                    if not sent:
                        BOT_STATUS["current"] = f"тЪая╕П Failed to send {tid}"
